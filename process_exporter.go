	...
	go func() {
		for {
			...
			for _, proc := range procs {
				...
				stimeGauge.WithLabelValues(labels...).Set(float64(stat.STime))
				cutimeGauge.WithLabelValues(labels...).Set(float64(stat.CUTime))
				cstimeGauge.WithLabelValues(labels...).Set(float64(stat.CSTime))
